...
#table(
  ...
  [Suspending function],[Profite du partage de thread. méthode suspending appelant autre suspending est mise en pause en attendant le retour.],
  [Blocking function],[opérations IO, longs calculs, ... Thread.sleep est bloquant (occupe la thread) mais pas suspending -> si job cancel, arrêt non immédiat, delay est suspending.],
  [Dispatchers],[Main : UI Thread, unique thread, Default : CPU, nb de threads = nb de CPU, IO : méthodes bloquantes, rapides, légères, nb de threads = dynamique, max 64],
